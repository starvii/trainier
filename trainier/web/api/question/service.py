...
class QuestionService:
    @staticmethod
    def __split_code(code: str) -> (int, str):
        """
        ...
        return QuestionService.__order_num(book, version, chapter, question_num)

    @staticmethod
    def __order_num(book: str, version: int, chapter: int, question_num: str) -> (int, str):
        """
    ...
    def select_next_prev_by_id(entity_id: str) -> (str, str):
        try:
            ...
            next_trunk: Trunk = Trunk.select(Trunk.entity_id).where((
                Trunk.parent_id.is_null() | Trunk.parent_id == '' | Trunk.parent_id == const.ROOT_NODE
            ), Trunk.order_num > order_num
            ).order_by(Trunk.order_num.asc()).first()
            prev_trunk: Trunk = Trunk.select(Trunk.entity_id).where((
                Trunk.parent_id.is_null() | Trunk.parent_id == '' | Trunk.parent_id == const.ROOT_NODE
            ), Trunk.order_num < order_num
            ).order_by(Trunk.order_num.desc()).first()
    ...
    def select_trunks(page: int, size: int, keyword: str, ids: List[str] = None) -> (List[Trunk], int):
        try:
            ...
            c: int = query.count()
            l: List[Trunk] = query.order_by(Trunk.order_num.asc()).paginate(page, size)
    ...
    def save(trunk: Trunk) -> None:
        ...
        try:
            while len(queue) > 0:
                ...
                trunk_cur.order_num, trunk_cur.code = QuestionService.__split_code(trunk_cur.code)  # 计算 order_num
                QuestionService.__save_trunk(trunk_cur)  # 保存后可获取 trunk_id
                if trunk_children is not None:
                    ...
                    for idx, option in enumerate(options):
                        ...
                        option.comment = ''
                    QuestionService.__save_options(options, trunk_cur)
        except Exception as e:
            Log.trainier.error(e)
            raise e

    @staticmethod
    def __save_trunk(trunk: Trunk) -> None:
